...
{
	name = "Item Give",
	author = "Zakikun&Disappear9&HMBSbige",
	description = "Item Give Menu",	
	version = VERSION,
...
{
	RegAdminCmd("sm_give", ItemGiveMenu, ADMFLAG_GENERIC, "Show menu");
	CreateConVar("item_give_version", VERSION, "The version of Item give", 270656);
	new Handle:hItemGiveMenuCFG = LoadGameConfigFile("item_give_fix");
	new Address:RFixAddr;
	if (hItemGiveMenuCFG)
	{
		RFixAddr = GameConfGetAddress(hItemGiveMenuCFG, "RYKnifeFix");
		StartPrepSDKCall(SDKCall_Player);
		PrepSDKCall_SetFromConf(hItemGiveMenuCFG, SDKFuncConfSource:1, "RoundRespawn");
	...
	CloseHandle(hItemGiveMenuCFG);
}

public Action:ItemGiveMenu(client, args)
{
...
public Action:ItemGive(clientId)
{
	new Handle:menu = CreateMenu(ItemGiveMenuHandler, MenuAction:28);
...
public ItemGiveMenuHandler(Handle:menu, MenuAction:action, client, itemNum)
{
