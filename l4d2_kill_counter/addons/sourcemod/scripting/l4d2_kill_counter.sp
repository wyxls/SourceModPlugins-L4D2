...
#define PLUGIN_PREFIX "\x04[统计] \x03"
...
{
	name = "Kill Counter",
	author = "HMBSbige && Zakikun",
	description = "Count kills and headshots",
...
public OnPluginStart()
{
	...
	g_hInterval = CreateConVar("sm_killcounter_ad_interval", "0.0", "Amount of seconds between advertisements. (0 = off)", FCVAR_NONE, true, 0.0);
...
public Action:Event_Player_Hurt(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	//Prevent frequent inform chat to attacker
	...
	new String:buf[128];
	Format(buf, 128, "\x04[友伤] \x05%N \x03打中了你%s.", attacker, hit);
	PrintToChat(client, buf);

	if ((ff_attack == 1) && (id != client))
	{
		PrintToChat(attacker, "\x04[友伤] \x03你误伤了\x05%N%s.", client, hit);
	}
