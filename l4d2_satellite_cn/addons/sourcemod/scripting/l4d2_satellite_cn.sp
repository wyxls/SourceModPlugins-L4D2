...
#define MESSAGE_SHIFT_JUDGEMENT		"切换模式: 审判"
#define MESSAGE_SHIFT_BLIZZARD		"切换模式: 冰冻"
#define MESSAGE_SHIFT_INFERNO		"切换模式: 炼狱"
#define MESSAGE_SHIFT_SLEEP			"切换模式: 关闭"
...
public ChangeMode(client)
{
	...
	Format(mStrJud, sizeof(mStrJud), "模式: 审判 (能量: %d)", energy[client][MODE_JUDGEMENT]);
	Format(mStrBli, sizeof(mStrBli), "模式: 冰冻 (能量: %d)", energy[client][MODE_BLIZZARD]);
	Format(mStrInf, sizeof(mStrInf), "模式: 炼狱 (能量: %d)", energy[client][MODE_INFERNO]);
	if (GetConVarInt(sm_satellite_adminunlimit) && HasSatelliteUnlimitAccess(client))
	{
		Format(mStrJud, sizeof(mStrJud), "模式: 审判 (能量: Unlimited)");
		Format(mStrBli, sizeof(mStrBli), "模式: 冰冻 (能量: Unlimited)");
		Format(mStrInf, sizeof(mStrInf), "模式: 炼狱 (能量: Unlimited)");
	}

	new Handle:menu = CreateMenu(ChangeModeMenu);
	SetMenuTitle(menu, "*** 卫星炮系统 ***");
	AddMenuItem(menu, "0", mStrJud);
	AddMenuItem(menu, "1", mStrBli);
	AddMenuItem(menu, "2", mStrInf);
	AddMenuItem(menu, "3", "关闭");
...
public Action:DisplayInstructorHint(Handle:timer, any:client)
{
	...
	DispatchKeyValue(entity, "hint_caption", "切换模式");
