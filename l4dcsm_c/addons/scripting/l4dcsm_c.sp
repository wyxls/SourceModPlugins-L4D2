...
{	
	...
	CreateConVar("l4d_csm_version", PLUGIN_VERSION, "Version of L4D Character Select Menu", FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY|FCVAR_DONTRECORD);
	...
	// Enable Cvar
	new Handle:Enabled = CreateConVar("l4d_csm_enable", "1", "Toggles the CSM plugin functionality", FCVAR_SPONLY, true, 0.0, true, 1.0);
	...
	// Announce cvar
	new Handle:Announce = CreateConVar("l4d_csm_announce", "1","Toggles the announcement of sm_csm command availability.",FCVAR_SPONLY,true, 0.0, true, 1.0);
	g_bAnnounce = GetConVarBool(Announce);
	HookConVarChange(Announce, _ConVarChange__Announce);
	
	// Survivor cvar
	new Handle:SurvivorModels = CreateConVar("l4d_csm_survivor_access", "1","1 = change to character (you become a clone). 2 = change model (look like the new character, but still your original character). 3 = Allow both 1 and 2.",FCVAR_SPONLY,true, 0.0, true, 3.0 );
	g_iSurvivorModels = GetConVarInt(SurvivorModels);
	HookConVarChange(SurvivorModels, _ConVarChange__SurvivorModels);
	
	// Infected cvar
	new Handle:InfectedModels = CreateConVar("l4d_csm_infected_access", "0", "1 = Can change character only. 2 = Change model only (look like the new character, but still your original character). 3 = Anybody can change character or model.", FCVAR_SPONLY, true, 0.0, true, 3.0);
	...
	{
		// L4D1 Survivors cvar
		new Handle:L4D1Survivors = CreateConVar("l4d_csm_l4d1_survivors", "0","Toggles access to L4D1 Survivors in L4D2. Set this to 1 if it's a L4D2 campaign, else if its a L4D1 Campaign, set this to 2 or glitches will occur",FCVAR_SPONLY,true, 0.0, true, 2.0);
		...
		new Handle:CommonModels = CreateConVar("l4d_csm_common_infected_models_access", "0", "Toggles access to Common Infected models", FCVAR_SPONLY, true, 0.0, true, 3.0);
		g_bCommonModels = GetConVarBool(InfectedModels);
		HookConVarChange(CommonModels, _ConVarChange__CommonModels);
	}
	
	// Change Limit Cvar
	new Handle:ChangeLimit = CreateConVar("l4d_csm_change_limit", "9999","Sets the number of times clients can change their character per life/map.",FCVAR_SPONLY,true, 0.0);
	g_iChangeLimit = GetConVarInt(ChangeLimit);
	HookConVarChange(ChangeLimit, _ConVarChange__ChangeLimit);
	
	//g_hCustomSurvivor1 = CreateConVar("l4d_csm_custom_survivor_1", "","Put the path for the custom survivor model here (models/survivors/survivormodel.mdl). SETTING THIS INCORRECTLY COULD CRASH THE SERVER! The map must be restarted the first time the server goes up.",FCVAR_SPONLY);
